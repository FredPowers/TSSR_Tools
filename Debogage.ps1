...
Do {
    ...
    write-host "2.  Comparer l'IP d'un hote entre le fichier host et le serveur DNS (Client)" -ForegroundColor DarkCyan
...
(New-Object System.Net.NetworkInformation.Ping).Send($GW) | fl Status, Address, RoundtripTime
        ...
        2 {
            ...
            write-host "#### Comparaison du fichier host et enregistrement du serveur DNS ####" -ForegroundColor Green
            Write-Host ""

            $Nom_Hote = Read-Host "Indiquer le nom d'hôte à vérifier"

            $Fichier_Host = Resolve-DnsName -Name $Nom_Hote

            $No_Fichier_Host = Resolve-DnsName -Name $Nom_Hote -NoHostsFile

            write-host "Adresse IP de $Nom_Hote dans le fichier host : $Fichier_Host"

            Write-Host ""

            Write-Host "Adresse IP de $Nom_Hote dans les enregistrements du serveur DNS : $No_Fichier_Host"
